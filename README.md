![](https://capsule-render.vercel.app/api?type=slice&color=ffc0cb&height=250&section=header&text=YeongGwangChoi&fontAlign=80&fontSize=40&fontColor=000000&animation=twinkling)
## Introduction

Hello, I am Yeonggwang Choi, an engineer dedicated to overcoming the limitations of autonomous driving through AI.

Currently, I am pursuing my Master’s degree at the Automation Lab at Sungkyunkwan University, focusing on autonomous driving, autonomous valet parking, and computer vision-based automation systems.

Since autonomous driving is a convergence of diverse technologies, I believe the role of an engineer who can translate research findings into real-world systems is paramount. I have always found excitement in challenges rather than fear, and I thrive on the growth that comes from such experiences. With this proactive mindset, I have published five papers in international conferences to build systematic academic expertise, while also accumulating practical development skills through various awards in autonomous driving competitions during my undergraduate years.

My key strength lies in accurately capturing rapidly evolving technological trends and swiftly applying new technologies to solve practical problems. This allows me to bridge the gap between research and development, implementing solutions that are immediately applicable in the field.

I am committed to leveraging my research expertise and development capabilities to become an engineer who turns future mobility technologies into reality.

If you are interested in my detailed profile, please visit my [portfolio](https://yeonggwangchoi.github.io/glory_portfolio/).


## Major
  - M.S. in Electrical and Computer Engineering
  - B.S. in Electronic Engineering
  - B.S. in Future Automotive Technology Integration

## Research Interests
  - Autonomous Driving & Valet Parking Systems
  - Computer Vision-based Automation
  - Deep Learning for Perception
  - Infrastructure Sensor-based Vehicle Control

My emails are:

  - dudrhkd7811@g.skku.edu


feel free to email me.


